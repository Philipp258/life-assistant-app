...
def _seed_label(slug: str, name: str | None = None) -> None:
    from app.db import SessionLocal
    from app.labels.models import Label

    with SessionLocal() as session:
        session.add(Label(slug=slug, name=name or slug))
        session.commit()


@pytest.mark.usefixtures("_test_db")
def test_do_create_task_accepts_labels():
    _seed_label("foo")
    out = do_create_task(title="Tagged", labels=["foo"])
    assert "error" not in out
    assert out["labels"] == ["foo"]


@pytest.mark.usefixtures("_test_db")
def test_do_create_task_defaults_to_empty_labels():
    out = do_create_task(title="Untagged")
    assert out["labels"] == []


@pytest.mark.usefixtures("_test_db")
def test_do_create_task_rejects_unknown_labels():
    out = do_create_task(title="Bad", labels=["does-not-exist"])
    assert "error" in out


@pytest.mark.usefixtures("_test_db")
def test_do_create_task_accepts_multiple_labels():
    _seed_label("foo")
    _seed_label("bar")
    out = do_create_task(title="Two-tagged", labels=["foo", "bar"])
    assert set(out["labels"]) == {"foo", "bar"}


@pytest.mark.usefixtures("_test_db")
def test_do_list_tasks_filters_by_labels_or_semantics():
    _seed_label("red")
    _seed_label("blue")
    _seed_label("green")
    do_create_task(title="r", labels=["red"])
    do_create_task(title="b", labels=["blue"])
    do_create_task(title="g", labels=["green"])

    out = do_list_tasks(labels=["red", "blue"])
    titles = sorted(t["title"] for t in out["tasks"])
    assert titles == ["b", "r"]


@pytest.mark.usefixtures("_test_db")
def test_do_list_tasks_unknown_label_filters_out_everything():
    do_create_task(title="any")
    assert do_list_tasks(labels=["nope"])["tasks"] == []


@pytest.mark.usefixtures("_test_db")
def test_do_update_task_replaces_label_set():
    _seed_label("one")
    _seed_label("two")
    task = do_create_task(title="x", labels=["one"])
    out = do_update_task(task["id"], labels=["two"])
    assert out["labels"] == ["two"]


@pytest.mark.usefixtures("_test_db")
def test_do_update_task_clears_labels_when_empty_list():
    _seed_label("one")
    task = do_create_task(title="x", labels=["one"])
    out = do_update_task(task["id"], labels=[])
    assert out["labels"] == []
